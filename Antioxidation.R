...
# Manuscript Title: Aflatoxigenic Aspergillus Modulates Aflatoxin-B1 Levels
# through an Antioxidative Mechanism
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

##################################################################
# [3.1] DETERMINATION OF ANTIOXIDATIVE DEGRADATION OF AFLATOXIN  #
##################################################################
...
# **************************************
# 1. Gene Expression due to Antioxidant:
# **************************************
...
p <- ggplot(modf) +
  geom_boxplot(aes(x=AOx, y=value, fill=variable)) + ggtitle("Structural (aflD) and Regulatory (aflR) Pathway Genes
Expression and Aflatoxin Response to Antioxidant

Isolate MLV14F ") + theme_classic() +
  labs(x= "Antioxidant (Se) Dose, µg/g", y = "Unit", fill="Legend", grouping("Gene")) + ylim(0, 17) + theme(plot.title = element_text(size = 13), axis.text = element_text(size = 13),text = element_text(size = 12)) # face = "bold")) #,face = "bold"))
...
##########################################################
# [3.2] FITNESS OF ISOLATES DUE TO ANTIOXIDANT TREATMENT # 
##########################################################
...
ggplot(fit, aes(x = AOx, y = cfu, col = Variant)) + theme_classic() + geom_boxplot(size=0.65, alpha=1) +
  labs(title = "  Role of Antioxidant (Se) in Fungal Darwinian Fitness", x = "Antioxidant (Se) Level, µg/g", y = y.expression) + ylim(0, 3) + theme_classic() +
  theme(axis.title.x = element_text(), axis.title.y = element_text(),
        panel.grid.major = element_blank(), panel.border = element_blank(),
        panel.background = element_blank(), axis.ticks = element_blank(), plot.title =  element_text(size = 11))

# Pairwise test, toxigenic vs atoxigenic Flavi due to antioxidant:
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
head(fit)
dim(fit)
treat <- subset(fit, AOx == "Se_0.86")

# NB:
# for other two lower antioxidant concentrations (0.4 mg/kg and 0 mg/kg) use:
# treat <- subset(fit, AOx == "Se_0.40")
# treat <- subset(fit, AOx == "Se_0.0")
